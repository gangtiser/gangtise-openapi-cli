...
- **流式输出**：`--format jsonl --output <file>` 时，翻页 / 全市场分片 / 逐只请求的行**按到达顺序逐批写盘**，取数阶段不再持有整份结果，内存不随行数增长（80 万行导出常驻约 150 MB）；不足 1000 行的结果仍走原来的整体写出。`csv --output` 超过 1000 行逐行写盘，但取数阶段仍在内存（列名要先取并集才能写表头），超大导出优先用 `jsonl`。
- **导出元信息**：`csv` / `jsonl` 落盘时旁边生成 `<文件>.meta.json`——命令行、行数、列名、`total` / `partial` / `failedPages` / `failedShards` / `truncatedShards` / `missingFields` 等全部完整性标记、抓取时间与时区、CLI 版本。这两种格式的文件本身只有数据行，转交之后靠它核验是否完整；`json` 自带标记，不生成。
