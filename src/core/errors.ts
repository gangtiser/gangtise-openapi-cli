...
const ERROR_HINTS: Record<string, string> = {
  ...
  // 只有 raw call 打错端点或服务端行为变化才可能撞上。实测 2026-09-12：方法不符落到
  // 900002（HTTP 405），请求体非法落到 100003 PARAM_INVALID（HTTP 400，带 traceId），
  // 而 Content-Type 不符回的是 HTTP 415 的框架原生错误页——没有信封，code / msg /
  // traceId 全无，按码匹配的提示到不了。此处为服务端接上新码后的预置。
