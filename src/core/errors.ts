...
const ERROR_HINTS: Record<string, string> = {
  ...
  // 只有 raw call 打错端点或服务端行为变化才可能撞上。实测 2026-09-24：Content-Type
  // 不符回 999008（HTTP 415）、请求体非法回 999009（HTTP 400），都带 errorType /
  // traceId；方法不符仍落到旧码 900002（HTTP 405），999007 为其接上新码后的预置。
  ...
  // 但历史上两者都归过此码，所以提示保留对 --file-type 的指引。云盘管理对不存在的
  // 文件夹 / 文件 ID 也回此码，包括重复执行一次已生效的删除。
  "130002": "确认 ID 有效且本账号可见（云盘的文件 / 文件夹 ID 用 `vault drive-folder-list` 核对；删除类命令收到此码可能是目标已被删掉）；下载类还需检查 --file-type 取值是否合法。",
  ...
  // 三类来源共用此码：AI 异步生成失败；EDE 的参数/表达式错误（实测 2026-08-02：
  // 枚举越界「参数 adjustType 的值 99 不在有效范围内 [1,2,3,4]」、表达式语法错误）；
  // 其他接口的 PROCESSING_FAILED「业务处理失败」（与入参无关）。都不该立即重试，所以
  // 文案按 msg 分流而不预设是哪一类。
  "140002": "终态失败，立即重试同一请求不会变。msg 指向参数（EDE 的指标参数名/枚举以 indicator search 的 parameterList 为准）就改参数重新提交；msg 是「业务处理失败」则与参数无关，稍后再试，持续出现请带 trace 报障。",
  ...
  "230003": "股票池数量已达账号上限（30 个）——先用 `vault stock-pool-delete` 删掉不用的池再建。",
  // 建/改股票池的两个专有码。名称重复与名称超长各占一个，所以「换个名字」和「缩短名字」
  // 两种动作不必靠猜（实测 2026-09-19：两者都带可读 msg，超长的 msg 还给出上限值）。
  "230006": "池名与该账号已有的池重复——先用 `vault stock-pool-list` 看现有名称，换一个再建/改名。",
  "230007": "池名超过 10 个字符（按字符计，中文算 1 个）——缩短后重试。终端里建的老池可以更长，OpenAPI 建池不行。",
  // 云盘管理的三个专有码（2026-09-23 文档）。名称按 UTF-16 计，emoji 算 2。
  "230004": "云盘文件 / 文件夹名称超过 200 个字符（按 UTF-16 计，中文算 1、emoji 算 2）——缩短 --name / --title 后重试。",
  ...
  // ── 旧码（一部分仍在线，一部分已被取代）──
  // 2026-09-12 实测：0000001007 / 0000001008 / 900002 仍是服务端当前吐的码；
  // 「缺必填项」已由 100001 PARAM_MISSING 承担，不再是 900001；错 AK/SK 登录返
  // 999011 CREDENTIAL_INVALID，不再是 8000014 / 8000015。全部保留作兜底。
  ...
  // EDE 专有旧码（410001 / 410106 / 410004）。2026-09-12 复测已不再出现——入参与
  // 表达式错走 100003 PARAM_INVALID（「指标 X 不存在」「指标 X 不支持参数 tradeDate」
  // 同码），指标必填参数缺失走 100001 PARAM_MISSING。保留兜底：错误码是按模块分批迁移
  // 的，回滚或未迁完的路径仍可能吐旧码，届时给条能用的提示比 fallthrough 强。
  // ⚠️ 紧随其后的 410110 / 410111 不在此列：异步内容端点仍在用它们（410110 实测
  // 2026-09-12 仍在线），别跟着当成已退役的码清理掉。
