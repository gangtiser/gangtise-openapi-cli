...
- ⚠️ **`scopeList` 是声明不是保证**：它可能**超前于数据**——`finc_pb_mrq` 声称覆盖 A股+港股，实测港股仍无数据（复测 2026-08-08）。反过来也发生过：`qte_mkt_cptl`/`shr_tot` 在 08-02 时声称覆盖港/美股却无数据，08-03 数据补上了。**2026-08-08 起判断成本降了**：无数据一律是 `null` 单元格（行列都保留），不再因「同批还查了什么」变成整列/整行消失，所以抽查一行就能直接看出覆盖情况（详见下方「缺数据 vs 代码写错」）
...
**缺数据一律是 `null` 单元格，行列都保留**，连单指标 × 单证券的 1×1 缺口也是 `null` 而不是空表。判据是**代码认不认识**，跟有没有数据无关：

| 情况 | 服务端怎么返 | CLI |
| :--- | :--- | :--- |
| 代码有效、但**无数据 / 无覆盖 / 非交易日 / 未来日期** | 单元格 `null`，行列都在 | **退出 0**，不标 partial |
...
**这是个净收益的变化**：`partial` / 退出码 3 从「这批数据不完整」变成了**「你有代码写错了」**——后者原本是完全静默的（退出 0、表看着正常、`--key-by code` 回填时 key 直接不存在）。反过来，真实的覆盖缺口现在一眼可见（就是 `null`），不再需要「和一个已知有数的标的一起查」这种对照法。
...
| `999999` | 系统故障。2026-08-01 起「无数据」不再用此码，所以它基本只剩真故障（2026-07-26 曾出现 EDE 取数端全线 999999，08-01 已恢复）。⚠️ 别把它和空表混为一谈：无数据现在是 `null` 单元格，空表表示整轴 code 未识别 | CLI 对 indicator 端点**不重试此码**（v0.27.0）；确认参数无误仍报错就是服务端问题 |
...
- **`scopeList` 声称覆盖 ≠ 真有数**：截至 2026-08-08，`finc_pb_mrq` 的 `scopeList` 写着 A股+港股，实测港股仍无数据。**2026-08-08 起这件事好查了**：无覆盖就是 `null` 单元格（行列都在，跟同批查了什么无关），抽查一行即可判定，不用再靠对照组反推。跨市场批量取数后**逐市场抽查一行**，别默认 scopeList 就是事实——反过来也一样，`qte_mkt_cptl`/`shr_tot` 在 08-02 还是港/美无数据、08-03 就补上了，**否定结论同样会过期**。
...
- **探索性取数**：缺值一律是 `null` 且保留行列（2026-08-08 起，含 1×1 的最简形态）；整列 / 整行消失现在只意味着**那个 code 服务端不认识**。看趋势用 `time-series` + 覆盖报告期的区间，但不能把缺值当成通过语义 / scope 校验。
...
- **空结果排查顺序**：2026-08-08 起真无数据会返回 `null` 单元格而不是空表，所以**空表基本等于「没有任何 code 被认出来」或参数名写错**。按序排查：① 证券代码与后缀对不对（美股 `.O`/`.N`，不是 `.US`）② 指标 code 拼写对不对 ③ 参数名对不对（`indicator search` 的 `parameterList`）④ 日期语义对不对（`tradeDate` vs `reportDate`）
