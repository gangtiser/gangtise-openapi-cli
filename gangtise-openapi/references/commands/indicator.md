...
- ⚠️ **`scopeList` 是声明不是保证**：它可能**超前于数据**——`finc_pb_mrq` 声称覆盖 A股+港股，实测港股无数据（2026-08-03）。反过来也发生过：`qte_mkt_cptl`/`shr_tot` 在 08-02 时声称覆盖港/美股却无数据，08-03 数据补上了。缺数据的**表现形式取决于同批里有没有别的标的有数**：和有数的标的一起查 → 那几格是 `null`（退出码 0）；只查缺的那些 → 整列消失或整表为空。所以「有 `null` 就说明覆盖了」是错的，批量前抽查一行
...
同一个覆盖缺口（`finc_pb_mrq` 无港股数据），会因**同批里还查了什么**而落进不同的档。2026-08-03 实测三种：
...
| 再 + `--indicator qte_close` | 回 2 行，泡泡玛特的 `市净率(MRQ)` 为 `null` | **退出 0**（第 ① 档：`qte_close` 保住了这一行，缺口降级成单元格） |

也就是说：**加一个有覆盖的指标，能把「整行消失」变回「`null` 单元格」**；反过来，只查一个没覆盖的指标就什么线索都没有。标 `partial` 的那档会在 stderr 打印被略过的 code（`--format json` 下 stdout 仍是干净 JSON）。脚本按 `!= 0` 判失败的要注意：3 表示「有数据但不完整」，不是硬失败。
...
- **`scopeList` 声称覆盖 ≠ 真有数**：截至 2026-08-03，`finc_pb_mrq` 的 `scopeList` 写着 A股+港股，实测港股无数据。它的表现按上面「缺数据的四种形态」走，取决于**同批还查了什么**：只查它 + 港股 → 整表为空（退出 0）；加一只 A 股 → 港股整行消失（退出 3）；再加一个有港股覆盖的指标（如 `qte_close`）→ 降级成 `null` 单元格（退出 0）。跨市场批量取数后**逐市场抽查一行**，别默认 scopeList 就是事实——反过来也一样，`qte_mkt_cptl`/`shr_tot` 在 08-02 还是港/美无数据、08-03 就补上了，**否定结论同样会过期**。
