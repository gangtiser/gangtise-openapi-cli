...
>
> ⚠️ **但 `15:00` 那根（收盘集合竞价）会随机返 `volume=0, amount=0`**，此时 Σ分钟比日线少约 1.2%。**这是逐只、逐日、且同一只票重复取也未必一致的**——同一个交易日里，有的标的每次都对得上（Σ分钟 `amount` 与日线只差浮点级），有的则时有时无。**所以不能拿一只票的一次结果推断当天整批数据**，也与「是不是最新交易日」无关。
>
