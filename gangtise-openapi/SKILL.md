...
- **退出码 3 = 结果不完整或不可信**（两种语义，都保留已取到的数据；stderr 有 warning，`--format json` 才看得见标记，table/csv/jsonl 只有数据行、看不出问题）：
  ...
  - **`screener` 的缺列分两档**：表达式**实际过滤用到**的变量缺列 → **退出码 1、不输出任何结果**（筛选移除的是证券不是指标，列没了说明那个条件根本无法证明被执行过，而返回的行却以「通过了它」的名义呈现）；只作输出、未参与表达式的辅助变量缺列 → `partial` + 退出码 3
