...
- **条件选股校验返回的变量绑定**。响应里每列带的 `field` 是**唯一**能把这一列追溯回它来自哪个筛选条件的东西，而载荷里没有任何别的信息能发现它漂了：实测把请求的 `F1` 换成 `F9` 返回，CLI 照样退出 0、stderr 全空、打出一张正常的选股表。现在只要**有证券命中**就要求：每个返回条目命名一个**请求过的**变量、带该变量对应的 code、不重复，且**表达式过滤用到的每个变量都必须有列**——否则**退出码 1**（这不是「少了行」也不是「值可疑」，是整份结果无法归属到条件，不该打印）。
  - **致命判定只对「纯合取」表达式生效**。`&&` 下每个引用变量都必须对每一行成立，缺列确实是无法证明的主张；但表达式含 `||` 时，一行完全可以经由另一个操作数正当命中，而这个操作数根本无法求值——实测 `F1 > 0 || F2 > 0` 扫 `09992.HK`（`finc_pe_ttm` 无港股覆盖），泡泡玛特靠 `F2 > 0` 正当命中，服务端返回的是**一份正确完整的答案**，早先的守卫却把它整份丢弃。含 `||` 时（包括 `F1 > 0 && (F2 > 0 || F3 > 0)` 这种混合式，不解析优先级、按不会丢正确答案的那一侧取）降级为 `partial` + 退出码 3，并在警告里点明「该条件对这些行一次都没应用上」。「PE 低 **或** 价格高」扫港美股是很普通的筛法，不是刁钻组合
  - 这两档差别写进了随包 skill（`SKILL.md` 的退出码 3 说明 + `indicator.md` 的缺数据表 + `response-schema.md` 的缺数据说明），并各有一条 E2E 钉住：过滤变量缺列 → 退出 1 且 stdout 为空；辅助变量缺列 → 退出 3 + `omittedIndicators` 且数据行照常输出
