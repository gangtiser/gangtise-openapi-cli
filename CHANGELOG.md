...
- **条件选股校验返回的变量绑定**。响应里每列带的 `field` 是**唯一**能把这一列追溯回它来自哪个筛选条件的东西，而载荷里没有任何别的信息能发现它漂了：实测把请求的 `F1` 换成 `F9` 返回，CLI 照样退出 0、stderr 全空、打出一张正常的选股表。现在每个返回条目都必须命名一个**请求过的**变量、且带该变量对应的 code、且不重复，否则**退出码 1**（这不是「少了行」也不是「值可疑」，是整份结果无法归属到条件——不该打印）。少返回某列不算失败（那可能只是被筛掉了），全空结果不绑定任何东西、照常退出 0
