...
- **screener 重复指标 → 标 `unreliable` + `duplicatedIndicators` + 退出码 3**。这类结果不是"少了行"而是"在场的值不可信"（被重复绑定的变量恒为 `null`，任何针对它的比较等于没筛），所以用 `unreliable` 而非 `partial`，`printData` 两者都触发退出码 3。**状态是「已检测并告警，等待服务端修复」，不是「已修复」**——服务端缺陷仍在，已报后台
...
- **`securityNameList` 异常不再静默错标，但也不会因此炸掉整条命令**。名称是**按位置**消费的：`["泡泡玛特"]` 配 `["600519.SH","09992.HK"]` 会把茅台的序列标成泡泡玛特，`[null]` 元素则渲染出一个字面量叫 `"null"` 的列（都是 2026-08-02 实测）。现在长度不符 → **整份丢弃名称、列头回落证券代码 + stderr 警告**；单个元素为 `null`/空 → 该列回落代码；非字符串元素 → 回落并警告一次。**有意与本模块其他守卫不对称**：那些守卫拦的是「值被错误归属」，必须致命；这一条拦的只是标题，而 `securityCodeList` 本身就承载身份——丢标题保数据，比连正确的数值一起毙掉更优
...
- **`cross-section` 的 `name` 列在没有名称列表时直接缺省**，不回落成证券代码——行轴 `security` 已经是代码，再复制一遍没有信息量。（时序的**列头**确实会回落到代码，两者是不同的东西）
...
- **必需轴缺失或类型错误不再透传成功**。透传条件写的是「完全没有轴字段」，实际却是「**任意**一个轴解析失败就透传」——于是时序返回完整的证券/指标/values 但 `dates: null`、或 screener 缺 `indicatorList`，都会原样打印原始信封、退出码 0、stderr 全空，本轮的形状保护照样被绕过。现在只有**一个矩阵字段都不带**的载荷才按「非 EDE 数据」透传；只要出现任一矩阵字段，该端点需要的每个轴（截面/选股：`securityCodeList` + `indicatorList` + `values`；时序另加 `dates`）都必须在场且为数组，否则抛形状错并带 traceId
...
- **`response-schema.md` 的 EDE 概述整段过期**：还写着「三个接口」「平行 `indicatorCodeList`/`indicatorNameList`」「截面 `[指标][证券]`」「无数据为 `null` 单元格」，与同文件后半段自相矛盾。改为四个接口、结构化 `indicatorList`、截面 `[证券][指标]`（并注明该版转置过），以及三档缺数据形态
...
- `examples.md` 的「单元格缺值返回 null 且不丢证券行」同步改为三档说明
- 删掉 `indicator.md` 里两处「港股市值/股本返回 `null`」的旧口径：实际表现是单查整体空表（退出 0）、混查整行/整列消失（退出 3），从来不是 `null`
- 说明 `cross-section` 的 `--date` 必填是 CLI 护栏而非协议要求：截面接受 `indicatorParamList: []`（`pty_op_scope` 实测照常返值），空 `parameters` 被丢弃是 **`screener` 独有**的缺陷。多带一个无害参数 vs 漏传触发空表，权衡不对称

**内部**
- **补顶层 `uncaughtException` / `unhandledRejection` 兜底**。此前 `main()` 的 try/catch 只罩住 `program.parseAsync`，事件回调里抛出的错误会走 Node 默认路径：一坨崩溃转储 + 非零退出——于是一条**数据已经正确打印完**的命令看起来像硬失败。`process.stdout` 的 error 处理器对非 EPIPE 错误正是 `throw`（管道竞态下的 `ERR_STREAM_DESTROYED`/`EBADF` 都会命中）。现在统一收敛成和其他错误一样的 `Name: message` + 退出码 1。本版主题是「退出码要可信」，这是唯一一条会反向说谎的路径
...
- 🔴 **条件选股的文本筛选（`contains`/`notcontains`）此前不可用**。根因不是「不支持 string 指标」，而是 **screener 会丢弃任何 `parameters` 为空数组的指标**：`pty_op_scope` 按官方格式传 `[]` 时 0 命中，挂上一个（对它无意义的）`tradeDate` 就正确返回。CLI 现在把 `--date` 无条件下发给每个指标，`--date` 相应改为**必填**——官方文档的招牌示例 `F3 contains '酒'` 现在能用了（白酒板块 19 只全部命中）
...
**文档订正（全部实测）**
- **「无数据返回 `null` 且不丢行」写反了**，这是最危险的一条。已改为三档说明（部分缺 → `null`／整指标缺 → 整列消失／整证券缺 → 整行消失／全空 → 空表），并标出各自后果
